...
export default function MarketPriceService() {
  const KEY = 'market_price_history';
  ...
  function send() {
    ...
    setTimeout(() => setMessages((m) => [...m, `Bot: Latest market price recorded.`]), 600);
    setInput('');
  }

  return (
    <div>
      <h2 className="text-2xl font-semibold mb-2">Market Price Updates</h2>

      <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-white rounded shadow-sm">
          <h3 className="font-medium">Today</h3>
          <p className="text-sm text-gray-600 mt-2">Wheat: $10/kg · Rice: $12/kg · Maize: $8/kg</p>
        </div>

        <div className="p-4 bg-white rounded shadow-sm">
          <h3 className="font-medium">Trends</h3>
          <p className="text-sm text-gray-600 mt-2">Prices rose 2% week-over-week in local markets.</p>
        </div>
      </div>
